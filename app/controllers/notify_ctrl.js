...
const NotifyModel = require('../models/notify_model');
...
class NotifyController extends Controller {
    ...
    async get(req, res) {
        try {
            ...
            const rs = await notifyModel.getAll();
    ...
    async add(req, res) {
        try {
            const notifyModel = new NotifyModel();
            const rs = await notifyModel.add(req.body);
            res.json(new Response(RESPONSE_STATUS.SUCCESS, rs))
        } catch (e) {
            res.json(new Response(RESPONSE_STATUS.ERROR, {}, { code: '400', message: 'Có lỗi xảy ra. Vui lòng liên hệ để được giải đáp.' }));
        }
    ...
    async update(req, res) {
        try {
            const notifyModel = new NotifyModel();
            const rs = await notifyModel.update(req.body)
            res.json(new Response(RESPONSE_STATUS.SUCCESS, rs))
        } catch (e) {
            res.json(new Response(RESPONSE_STATUS.ERROR, {}, { code: '400', message: 'Có lỗi xảy ra. Vui lòng liên hệ để được giải đáp.' }));
        }
    ...
    async remove(req, res) {
        try {
            const notifyModel = new NotifyModel();
            const rs = await notifyModel.del(req.params.id);
            res.json(new Response(RESPONSE_STATUS.SUCCESS, rs))
        } catch (e) {
            res.json(new Response(RESPONSE_STATUS.ERROR, {}, { code: '400', message: 'Có lỗi xảy ra. Vui lòng liên hệ để được giải đáp.' }));
        }
